@isset($sessions)
    @if(count($sessions))
        <table class="{{config('ksoft.style.table_style')}}">
            ...
            <tbody>
                @foreach($sessions as $session)
                    ...
                    $ante = Arr::get($payload,'_previous');
                    $referer = Arr::get($payload,'referer');
                        ...
                        <td>
                            @if ($referer)
                                <a href="{{ $referer}}" data-toggle="tooltip" title="{{$referer}}" target="_blank">
                                    <i class="far fa-external-link-square-alt fa-fw mr-1"></i> {{Str::limit(last(explode('/', $referer)),34)}}
                                </a>
                            @endif
                        </td>
                        <td>
        ...
        {{-- <a href="{{ action([\App\Http\Controllers\Back\ActivitylogController::class, 'index']) }}">
