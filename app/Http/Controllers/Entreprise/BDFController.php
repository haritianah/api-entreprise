...
class BDFController extends Controller
{
    public function index($siret)
    {
        ...
        if (!$res = Cache::get("{$siret}.get_BDF_status")) {
            $res = Cache::rememberForever("{$siret}.get_BDF_status", function () use ($siret) {
                $headers = [
                ...
                try {
                    $response = $client->request('GET', '/' . $version . '/bilans_entreprises_bdf/' . $sirenToSearch, [
                        'headers' => $headers,
